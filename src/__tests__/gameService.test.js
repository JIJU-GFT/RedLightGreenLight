...
test('timer is in range with 0 score', () => {
  ...
  expect(gameService.timeoutGreenLight).toBeGreaterThanOrEqual(
    LOW_POINTS_MIN_RANDOM
  );
...
test('can replace greenLight persistent data', () => {
  ...
  expect(localStorage.setItem).toHaveBeenCalledWith('greenLight', false);
});
...
test('can replace redLight persistent data', () => {
  ...
  expect(localStorage.setItem).toHaveBeenCalledWith('greenLight', true);
});

test('can stop green timer', () => {
  const gameService = new GameService(HIGH_POINTS, true);
...
test('can stop red timer', () => {
  const gameService = new GameService(LOW_POINTS, true);
  gameService.stopRedTimer();

  expect(setTimeout).toHaveBeenCalledWith(
    expect.any(Function),
    gameService.timeoutGreenLight
  );
});
